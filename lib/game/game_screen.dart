          ...
          return Scaffold(
              ...
              body: Stack(children: [
                ...
                Positioned(
                    ...
                    right: 0,
                    top: 40,
                    child: _ScoreCards(players: players)),
                Positioned(
                    left: 0,
                    right: 0,
                    bottom: 0,
                    child: _Buttons(players: players)),
              ]));
        },
        error: (st, er) {
...
class _ScoreCards extends ConsumerWidget {
  const _ScoreCards({required this.players});
  final List players;
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      ...
      return ListTile(
          tileColor:
              ref.watch(currentPlayerProvider) == index ? Colors.amber : null,
            ...
            children: List.generate(25, (boxindex) {
              return Padding(
                ...
                child: DecoratedBox(
                  ...
                  child: _BoxIcon(currentPlayer: index, currentTurn: boxindex),
                ),
              );
            }),
          ));
...
class _BoxIcon extends ConsumerWidget {
  const _BoxIcon({required this.currentPlayer, required this.currentTurn});
...
class _Buttons extends ConsumerWidget {
  const _Buttons({required this.players});
...
enum _ActionButtons {
  ...
  second(label: 'second', icondata: Icons.looks_two_rounded),
